...
export default function RegisterTechnician() {
    ...
    const [workImage, setWorkImage] = useState<ImageType | null>(null);
    const [workRatio, setWorkRatio] = useState(1);
    ...
    const pickImage = async (setImage: any, setRatio?: any) => {
        const result = await ImagePicker.launchImageLibraryAsync({
            ...
            allowsEditing: true,
            quality: 0.4, // Optimized quality to stay under Firestore's 1MB limit
            base64: true, 
        });
    ...
    const handleNext = () => {
        if (step === 1) {
            ...
            if (!ktpImage || !workImage) {
                alert("Harap upload foto KTP dan bukti kerja");
                return;
            }
            if (!sopChecked) {
        ...
        try {
            ...
            const response = await fetch(`${BASE_URL}/technician/register`, {
                ...
                body: JSON.stringify({
                    ...
                    bankName,
                    bankAccount,
                    ktpBase64: ktpImage.base64, // Send raw base64
                    workBase64: workImage.base64, // Send raw base64
                }),
            });
                    ...
                    <View style={styles.stepContainer}>
                        ...
                        <View style={styles.uploadContainer}>
                            <View style={styles.uploadRow}>
                                ...
                                <TouchableOpacity style={styles.uploadBox} onPress={() => pickImage(setWorkImage)}>
                                    {workImage ? (
                                        <Image source={{ uri: workImage.uri }} style={styles.uploadedImage} />
                                    ) : (
                                        <View style={{ alignItems: 'center' }}>
                                            <Ionicons name="image-outline" size={28} color="#B3875E" />
                                            <Text style={styles.uploadTitle}>Foto Bukti Kerja</Text>
                                            <Text style={styles.uploadSub}>(Sertifikat/Foto di lapangan)</Text>
                                        </View>
                                    )}
                                </TouchableOpacity>
                            </View>
...
const styles = StyleSheet.create({
    ...
    uploadedImage: {
        width: 140, // Fixed width for reliability
        height: 100, // Fixed height for reliability
        borderRadius: 8,
        resizeMode: "cover", // Cover looks better for KTP preview
        marginVertical: 5,
    },
    uploadDivider: {
        width: 1,
